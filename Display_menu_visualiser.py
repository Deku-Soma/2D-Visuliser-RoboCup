...
for x in range(0, 2):
    if x == 0:
        for i in range(0, 11):
            playerIM[i] = canvas_visualiser.create_image(teams[x].team[i].x(), teams[x].team[i].y(),
                                                         image=teams[x].team[i].get_sprite_image(), anchor=NW)
            # cts = str(ct)
            # canvas.tag_bind(playerIM,"<Button-"+cts+">", key)
    else:
        for i in range(0, 11):
            playerIM[i + 11] = canvas_visualiser.create_image(teams[x].team[i].x(), teams[x].team[i].y(),
                                                              image=teams[x].team[i].get_sprite_image(), anchor=NW)
            # cts = str(ct)
            # canvas.tag_bind(playerIM,"<Button-"+cts+">", key)
